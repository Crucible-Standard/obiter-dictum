...
function getChartTwo(req) {
  ...
  return new Promise(async (resolve) => {
    ...
    if (
      ...
      const args = req.query.ticker || req.body.text;
      const returnstring = `https://charts-node.finviz.com/chart?w=466&h=219&bw=2&bm=1&bb=1&tf=i5&s=linear&pm=0&am=0&ct=candle_stick&o[0][ot]=sma&o[0][op]=20&o[0][oc]=DC32B363&o[1][ot]=sma&o[1][op]=50&o[1][oc]=FF8F33C6&o[2][ot]=sma&o[2][op]=200&o[2][oc]=DCB3326D&o[3][ot]=patterns&o[3][op]=&o[3][oc]=000&t=${args}&rr=${new Date().getTime()}`;
  ...
  return new Promise(async (resolve) => {
    ...
    if (
      ...
      try {
        ...
        const sma20a =
          stock.sma20 > 0
            ? `${Math.floor(stock.price + stock.sma20)} (${stock.sma20}) 🚩`
            : `${Math.floor(stock.price + stock.sma20)} (${stock.sma20})  ✅`;
        const sma50a =
          stock.sma50 > 0
            ? `${Math.floor(stock.price + stock.sma50)} (${stock.sma50}) 🚩`
            : `${Math.floor(stock.price + stock.sma50)} (${stock.sma50})  ✅`;
        const sma200a =
          stock.sma200 > 0
            ? `${Math.floor(stock.price + stock.sma200)} (${stock.sma200}) 🚩`
            : `${Math.floor(stock.price + stock.sma200)} (${stock.sma200})  ✅`;
